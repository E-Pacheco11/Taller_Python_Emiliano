...
class ConexionMysql:
    ...
    def Insertar(self, tabla, valores)->bool:
        sql = f"INSERT INTO {tabla} (nombre, ap, edad) VALUES (%s, %s, %s)" #Modificar, agregar campos faltantes
...
if __name__ == '__main__':
    conexion=ConexionMysql("127.0.0.1",3306,"almacen","root","")
    ...
    if conexion.BanConexion:
         #if conexion.Insertar("usuarios",("Cesar Contreras Polanco", "CesarCont12", "cesarccontreras12@smartin.tecnm.mx","xxxpeggy12","1")):
        ...
        if res != None:
            for fila in res:
                print(f"ID: {fila[0]}, Nombre: {fila[1]}, Nombre de usuario: {fila[2]}, Correo electronico: {fila[3]}, Contraseña: {fila[4]}, Id Rol:{fila[5]}")
        else:
